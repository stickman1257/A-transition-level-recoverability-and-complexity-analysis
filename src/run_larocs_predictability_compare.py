...
from run_next_window import normalize_predictor_model_type
from run_predictability_model_compare import build_markdown_summary, load_json
from run_predictability_suite import run_predictability_suite
from utils import normalize_stage_name


DEFAULT_LAROCS_INPUT_STAGES = [
    "raw",
    "notch",
    "rectified",
]
DEFAULT_LAROCS_MODEL_TYPES = [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Larocs-style predictability comparison for "
            "raw/notch/rectified inputs and an lp_10hz target."
        )
    )
    ...
    parser.add_argument(
        "--input-stages",
        nargs="*",
        default=list(DEFAULT_LAROCS_INPUT_STAGES),
        help="Input stages to compare. Defaults to raw, notch, rectified.",
    )
    ...
    parser.add_argument(
        "--model-types",
        nargs="*",
        default=list(DEFAULT_LAROCS_MODEL_TYPES),
        help="Larocs-style models to compare.",
    )
...
def main() -> None:
    ...
    for model_type in args.model_types:
        ...
        print(f"\n===== Running Larocs comparison model: {model_type} =====")
